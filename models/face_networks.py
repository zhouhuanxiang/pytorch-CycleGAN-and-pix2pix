...
def define_G(class_n, input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        net = ResnetGenerator(class_n, input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
    elif netG == 'resnet_6blocks':
        net = ResnetGenerator(class_n, input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    elif netG == 'unet_128':
...
class ResnetGenerator(nn.Module):
    ...
    def set_requires_grad(self, iden_grad=True, attr_grad=True, gen_grad=True):
        ...
        for param in self.model2.parameters():
            param.requires_grad = attr_grad
        for param in self.model.parameters():
            param.requires_grad = gen_grad
